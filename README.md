...
### Options
 ...
 - `addEmptySchema`: a boolean that keeps the status codes of responses which declare no body - no `content` (v3) or no `schema` (v2) - instead of dropping them from the route schema. This might be useful for status codes like 204 or 304. Such a response is registered with a `{ "type": "null" }` schema, which is how Fastify spells "this response carries no body": it is serialized without a body, and a document regenerated from the routes by [@fastify/swagger](https://github.com/fastify/fastify-swagger) reports the status code again with no body. A v3 response which lists content types but no schema for them - a binary download, say - does carry a body, and keeps the `{}` schema which serializes anything. Default is `false`.
