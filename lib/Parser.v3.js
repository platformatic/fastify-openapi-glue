...
export class ParserV3 extends ParserBase {
  parseQueryString (data) {
    if (data.length === 1) {
      if (typeof data[0].content === 'object') {
        return this.parseContent(data[0])
      }
  ...
  parseResponses (responses) {
    ...
    for (const httpCode in responses) {
      const response = responses[httpCode]
      const body = this.parseContent(response)
      if (body !== undefined) {
        result[httpCode] = body
        continue
      }
      if (this.options.addEmptySchema) {
        result[httpCode] = hasContent(response) ? {} : this.bodylessSchema(response)
      }
    }
    return result
  }
